...
import { POLITICAL_BELIEF_OPTIONS } from '../../lib/profileOptions';

export default function PoliticalBeliefsFilter({ value, onChange }) {
  ...
  const label = useMemo(() => {
    if (!active) return 'Political Beliefs';

    if (value.length <= 2) {
      return value.join(', ');
    }

    return `${value.length} beliefs`;
  }, [active, value]);

  const toggleOption = (belief) => {
    setLocalValue((prev) => (
        ...
        >
          <div className="px-4 pt-4 pb-2">
            ...
            <p className="text-sm text-gray-600">
              Show people with these political beliefs.
            </p>
          </div>
