...
import { ETHNICITY_OPTIONS } from '../../lib/profileOptions';
import DealbreakerToggle from './DealbreakerToggle';

export default function EthnicityFilter({ value, onChange, dealbreaker = false, onDealbreakerChange }) {
  ...
  const label = useMemo(() => {
    if (!active) return 'Ethnicity';

    if (value.length <= 2) {
      return value.join(', ');
    }

    return `${value.length} ethnicities`;
  }, [active, value]);

  const toggleOption = (ethnicity) => {
    setLocalValue((prev) => (
        ...
        >
          <div className="px-4 pt-4 pb-2">
            ...
            <p className="text-sm text-gray-600">
              Show people with these ethnicities.
            </p>
          </div>
