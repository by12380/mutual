...
const formatSectionLabel = (sectionId) => {
  ...
  if (sectionId === 'interests') return 'Interests';
  if (sectionId.startsWith('photo-')) return `Photo ${sectionId.split('-')[1]}`;
  if (sectionId.startsWith('prompt-')) return `Prompt ${sectionId.split('-')[1]}`;
  return sectionId;
};
        ...
        <div className="bg-primary-50 border border-primary-100 rounded-xl p-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {cardLikes.map((cl) => (
              <span
                key={cl.sectionId}
                className="inline-flex items-center gap-1 px-2 py-0.5 bg-pink-100 text-pink-700 rounded-full text-xs font-medium"
              >
                <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
                </svg>
                {formatSectionLabel(cl.sectionId)}
              </span>
            ))}
            {comments.map((c, i) => (
              <span
                key={`comment-${c.sectionId}-${i}`}
              ...
              >
                ...
                {formatSectionLabel(c.sectionId)}: "{c.body.length > 30 ? c.body.slice(0, 30) + '...' : c.body}"
              </span>
            ))}
          </div>
        </div>
